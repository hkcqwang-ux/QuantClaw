...
int SkillHotInstall::ScanForNewSkills() {
  ...
  // Append new metas to the known list
  SkillMetaDiscoveredCallback cb_copy;
  {
    ...
    for (auto& meta : new_metas) {
      ...
      // Fire callback (while holding metas_mutex_ for consistency)
      {
        ...
        for (const auto& entry :
          ...
          if (is_new) {
            ...
            try {
              ...
              const SkillMetadata* meta_ptr = meta_loader_.FindMetaDataByName(path_str);
              if (meta_ptr == nullptr) {
              ...
              bool should_add = false;
              {
                ...
                if (!known_names_.count(meta_ptr->mini.name)) {
                  auto cfg_it = skills_config_.entries.find(meta_ptr->mini.name);
                  if (cfg_it == skills_config_.entries.end() ||
                      cfg_it->second.enabled) {
                    known_names_.insert(meta_ptr->mini.name);

                    known_metas_.push_back(*meta_ptr);
